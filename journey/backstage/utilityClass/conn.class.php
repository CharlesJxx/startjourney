...
class connMongodb{
    ...
    function getInstance($mongo_server, $flag=array()){
    	...
    	if(empty($flag['tag'])){
    		...
    		if(isset($flag['force']) && $flag['force'] == true){
    			$mongo = new connMongodb($mongo_server);
    ...
    * 
    * 返回值： 
    * 成功：修改后的记录(new=>true)或 修改前的记录(new=>false),默认为true
    * 失败：false 
    */ 
    function findAndModify($table_name, $condition, $modifydata, $options=array()){
    	$dbname = $this->curr_db_name;
    	$options['new'] = true;
    	$result = array();
    	try{
    		$result = $this->mongo->$dbname->$table_name->findAndModify($condition, $modifydata, null, $options);
    	}catch(MongoCursorException $e){
    		$this->error = $e->getMessage();
    		return false;
    	}
    ...
    * 失败：false 
    */
    function find($table_name, $query_condition=array(), $result_condition=array(), $fields=array()){
    	$dbname = $this->curr_db_name;
    ...
    *失败：false
    **/
    function getAutoIncreId($incre_table, $name){
    	$result = array();
    	try{
    		$result = $this->findAndModify($incre_table, array("name"=>$name), array("\$inc"=>array("value"=>1)));
    	}catch(MongoCursorException $e){
    		$this->error = $e->getMessage();
    		return false;
    	}
